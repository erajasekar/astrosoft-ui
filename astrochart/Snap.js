...
// └────────────────────────────────────────────────────────────┘ \\
/* eslint-disable */
export default Snap(function (glob) {
    const version = '0.4.2'
...
const mina = (function (eve) {
 ...
 if (animations.hasOwnProperty(i)) {
     ...
     **
     ...
     o }
    \ */
    var mina = function (a, A, b, B, get, set, easing) {
        const anim = {
            id: ID(),
            start: a,
            end: A,
 ...
 if (animations.hasOwnProperty(i)) {
    ...
    mina.elastic = function (n) {
        ...
        return 2 ** (-10 * n) * Math.sin((n - 0.075) *
            (2 * Math.PI) / 0.3) + 1
    }
    /* \
     * mina.bounce
...
const has = 'hasOwnProperty'
    ...
    const reURLValue = /^url\(#?([^)]+)\)$/
    const spaces = '\x09\x0a\x0b\x0c\x0d\x20\xa0\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u202f\u205f\u3000\u2028\u2029'
    const separator = new RegExp('[,' + spaces + ']+')
    const whitespace = new RegExp('[' + spaces + ']', 'g')
...
function is (o, type) {
    ...
    if (type == 'array' &&
        (Array.isArray(o) || Array.isArray && Array.isArray(o))) {
        return true
    }
...
function extractTransform (el, tstr) {
    ...
    const m = transform2matrix(tstr, el.getBBox(1))
    if (doReturn) {
        return m
    } else {
        el.matrix = m
...
function getSomeDefs (el) {
    const cache = Snap._.someDefs
    if (cache && contains(cache.ownerDocument.documentElement, cache)) {
        return cache
    }
    const p = (el.node.ownerSVGElement && wrap(el.node.ownerSVGElement)) ||
...
function unit2px (el, name, value) {
    const defs = getSomeDefs(el)
        let out = {}
        let mgr = defs.querySelector('.svg---mgr')
    if (!mgr) {
        mgr = $('rect')
...
// SIERRA Element.transform(): Unclear how Snap transform string format differs from SVG's.
    ...
    elproto.transform = function (tstr) {
        const _ = this._
        if (tstr == null) {
            const global = new Matrix(this.node.getCTM())
                const local = extractTransform(this)
                const localString = local.toTransformString()
                const string = Str(local) == Str(this.matrix)
...
// SIERRA Element.marker(): I suggest the method should accept default reference point values.  Perhaps centered with (refX = width/2) and (refY = height/2)? Also, couldn't it assume the element's current _width_ and _height_? And please specify what _x_ and _y_ mean: offsets? If so, from where?  Couldn't they also be assigned default values?
 ...
 if (el.anims[has](id)) {
    ...
    elproto.animate = function (attrs, ms, easing, callback) {
        ...
        let fkeys = []; let tkeys = []; const keys = {}; let from; let to; let f; let eq
            const el = this
...
function gradient (defs, str) {
    const grad = arrayFirstValue(eve('snap.util.grad.parse', null, str))
        let el
    if (!grad) {
        return null
    }
...
function gradientLinear (defs, x1, y1, x2, y2) {
    const el = make('linearGradient', defs)
    el.stops = Gstops
    el.addStop = GaddStop
    el.getBBox = GgetBBox
    if (x1 != null) {
...
function gradientRadial (defs, cx, cy, r, fx, fy) {
    const el = make('radialGradient', defs)
    el.stops = Gstops
    el.addStop = GaddStop
    el.getBBox = GgetBBox
    if (cx != null) {
...
(function (proto) {
    ...
    proto.ellipse = function (cx, cy, rx, ry) {
        const el = make('ellipse', this.node)
        if (is(cx, 'object') && 'cx' in cx) {
            el.attr(cx)
        } else if (cx != null) {
            el.attr({
...
// SIERRA Paper.g(): Don't understand the code comment about the order being _different._ Wouldn't it be a rect followed by a circle?
 ...
 if (proto[has](method)) {
    ...
    proto.polyline = function (points) {
        ...
        const el = make('polyline', this.node)
    ...
    proto.polygon = function (points) {
        ...
        const el = make('polygon', this.node)
        if (is(points, 'object') && !is(points, 'array')) {
            el.attr(points)
        } else if (points != null) {
            el.attr({
    ...
    (function () {
        ...
        proto.toString = function () {
            const f = glob.doc.createDocumentFragment()
                const d = glob.doc.createElement('div')
...
function fillStroke (name) {
    return function (value) {
        ...
        if (value instanceof Element) {
            ...
            if (fill.error) {
                const grad = gradient(getSomeDefs(this), value)
                if (grad) {
                    if (!grad.node.id) {
                        $(grad.node, {
                            id: grad.id
...
eve.on('snap.util.getattr.viewBox', function () {
    eve.stop()
    const vb = $(this.node, 'viewBox').split(separator)
 ...
 if (p[has](key) && key != ps) {
    ...
    function findDotsAtSegment (p1x, p1y, c1x, c1y, c2x, c2y, p2x, p2y, t) {
        const t1 = 1 - t
            ...
            const alpha = (90 - math.atan2(mx - nx, my - ny) * 180 / PI)
    ...
    function ellipsePath (x, y, rx, ry, a) {
        ...
        if (a != null) {
            const rad = Math.PI / 180
                ...
                var res = [['M', x1, y1], ['A', rx, rx, 0, +(a - ry > 180), 0, x2, y2]]
        } else {
            res = [
                ['M', x, y],
                ['m', 0, -ry],
...
Snap.plugin(function (Snap, Element, Paper, glob) {
    ...
    function getPath (path) {
        let k = 0; let i; let ii; let j; let jj; let out; let a; const b = []
        for (i = 0, ii = path.length; i < ii; i++) {
            out = '['
            a = ['"' + path[i][0] + '"']
    ...
    Element.prototype.equal = function (name, b) {
        let A; let B; const a = Str(this.attr(name) || '')
            const el = this
        if (a == +a && b == +b) {
            return {
                from: +a,
...
Snap.plugin(function (Snap, Element, Paper, glob) {
    const elproto = Element.prototype
    const has = 'hasOwnProperty'
    const supportsTouch = 'createTouch' in glob.doc
    const events = [
    ...
    const addEvent = (function () {
        if (glob.doc.addEventListener) {
            return function (obj, type, fn, element) {
                const realName = supportsTouch && touchMap[type] ? touchMap[type] : type
                    const f = function (e) {
...
// SIERRA Paper.filter(): I don't understand the note. Does that mean an HTML should dedicate a separate SVG region for a filter definition? What's the advantage over a DEFS?
    ...
    pproto.filter = function (filstr) {
        ...
        const f = Snap.parse(Str(filstr))
            const id = Snap._.id()
            const width = paper.node.offsetWidth
            const height = paper.node.offsetHeight
            const filter = $('filter')
        $(filter, {
            id,
